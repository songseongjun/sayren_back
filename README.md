...
[![Portfolio](https://img.shields.io/badge/Portfolio-SAYREN-6C63FF?style=for-the-badge)](https://seongjun-portfolio.vercel.app)
[![Frontend](https://img.shields.io/badge/GitHub-Frontend-181717?style=for-the-badge&logo=github)](https://github.com/songseongjun/sayren_front)

**담당 영역:** OrderPlan · Cart · Order · OrderItem · OrderHistory · Address · Delivery · Notification  
**핵심 구현:** 주문 가격 Snapshot · 주문 상태 이력 · Spring Event · 배송/회수 상태 전환 · Next.js REST API 연동
...
**상품 선택 → 요금제 선택 → 장바구니/바로구매 → 주문 → 결제 → 배송 → 회수 → 알림**

으로 이어지는 커머스 흐름을 구현한 팀 프로젝트입니다.
...
제가 담당한 영역에서 특히 다음 부분을 중심으로 구현했습니다.

| 핵심 구현 | 내용 |
| --- | --- |
| 주문 가격 Snapshot | 주문 생성 당시 상품 가격을 별도 저장하여 과거 주문 금액 정합성 유지 |
| 주문 상태 이력 | `OrderHistory`를 이용해 현재 상태와 상태 변경 기록 분리 |
| 배송 상태 관리 | `READY → SHIPPING → DELIVERED` 상태 흐름 관리 |
| 회수 상태 관리 | `RETURN_READY → IN_RETURNING → RETURNED` 상태 흐름 관리 |
| Event Driven Flow | 주문·결제·배송·알림 간 후속 로직을 Spring Event 기반으로 연결 |
| REST API | 장바구니·주문·배송지·배송 관련 Backend API 구현 |
| Frontend Integration | Next.js / Axios / TanStack Query 기반 API 연동 및 캐시 갱신 |
...
팀 프로젝트에서 아래 도메인의  
**Backend 비즈니스 로직, REST API 및 Frontend 연동**을 담당했습니다.
...
- 장바구니 및 주문 비즈니스 로직 구현
- 주문·배송 상태 변경 로직 구현
- 주문 상태 변경 이력 관리
- 배송 및 회수 상태 전환 관리
- Spring Event 기반 도메인 후속 처리 연동
- Next.js Frontend와 REST API 연동
- TanStack Query 기반 서버 상태 관리 및 Query Cache 갱신
...
렌탈 상품은 상품 선택 단계에서 기간별 요금제를 선택할 수 있도록 구성했습니다.
...
장바구니와 주문에서 사용자가 어떤 상품을 어떤 조건으로 선택했는지 식별할 수 있도록 구현했습니다.
...
- 일반 구매 상품과 렌탈 상품을 하나의 장바구니에서 관리
- 주문 생성 후 장바구니 정리
...
주문이 `Product`의 현재 가격만 참조하면  
상품 가격이 변경되었을 때 기존 주문의 금액까지 영향을 받을 수 있습니다.
...
상품 가격이 이후 변경되더라도  
이미 생성된 주문은 **주문 당시 가격을 그대로 유지**하도록 구성하여 과거 주문 데이터의 금액 정합성을 유지했습니다.

---

# 📦 배송지 - Address

Checkout 과정에서 사용자가 배송지를 선택하거나 신규 등록할 수 있도록 배송지 영역을 구현했습니다.
...
주문 이후 실제 상품의 배송과 렌탈 상품의 회수 흐름을 관리합니다.
...
정해진 생명주기에 따라 상태가 변경되도록 구성했습니다.

---

# 🔗 Payment → Delivery 연동

결제 `Payment` 도메인은 팀원이 담당했으며,  
제가 담당한 Delivery 도메인과의 연동은 **결제 상태 변경 이벤트**를 기준으로 처리했습니다.
...
을 생성하고 초기 배송 상태를 `READY`로 설정하도록 연결했습니다.

이를 통해 Payment Service와 Delivery Service가 서로 직접 강하게 결합되지 않도록 하고,  
**상태 변경 이벤트를 기준으로 후속 로직이 실행되는 구조**로 구성했습니다.

---

# 🔔 Notification

주문·결제·배송 상태 변화가 사용자 알림으로 이어질 수 있도록  
...
도메인 간 직접 호출을 줄이고  
상태 변화 이후 필요한 후속 작업을 이벤트 기반으로 분리했습니다.

---

# 🔄 Event Driven Flow

담당 도메인은 단순 CRUD만 구현하는 방식이 아니라

**상태 변경 → 이력 기록 → 이벤트 발행 → 후속 처리**
...
이를 통해 주문·결제·배송·알림 영역이 서로 직접 강하게 결합되는 것을 줄이고  
각 도메인의 책임을 분리했습니다.
...
Controller → Service → Repository / Mapper 계층을 기준으로 역할을 분리했습니다.
...
주문 취소 시에는 상태를 `CANCELED`로 변경합니다.
...
현재 상태는 `Order`에서 관리하고,  
상태가 변경된 기록은 `OrderHistory`에서 별도로 관리했습니다.

이를 통해 **현재 상태와 과거 상태 변경 기록을 분리**했습니다.
...
상품 가격이 이후 변경되더라도  
이미 생성된 주문은 주문 당시 금액을 유지할 수 있도록 했습니다.
...
Order, Payment, Delivery Service가 서로 직접 호출하면  
...
TanStack Query Mutation 성공 이후  
관련 Query를 invalidate하도록 처리했습니다.
...
담당 Backend REST API를 Next.js Frontend와 연결하여 다음 기능과 연동했습니다.
...
해당 도메인과 제가 담당한 영역이 연결되는 부분은  
Entity 관계와 상태 변경 이벤트를 이용하여 연동했습니다.
...
> **"데이터가 어떤 흐름으로 이동하고, 상태가 왜 변경되며, 다른 도메인에 어떤 영향을 주는가"**

를 고려하며 구현하는 경험을 할 수 있었습니다.

---

### 🔗 Links

[![Portfolio](https://img.shields.io/badge/Portfolio-Visit-6C63FF?style=for-the-badge)](https://seongjun-portfolio.vercel.app)
[![Frontend](https://img.shields.io/badge/GitHub-Frontend-181717?style=for-the-badge&logo=github)](https://github.com/songseongjun/sayren_front)
